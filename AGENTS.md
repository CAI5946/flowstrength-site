...
- Structure the page in this order: homepage, feature gallery, user pain points, competitor analysis, product decisions, AI system design, AI evaluation, user testing, launch operations, and project reflection.
...
- Treat desktop as the primary portfolio viewport. Keep the competitor flow as a compact three-lane desktop comparison with only `数据输入 → 判断方式 → 调整落地` plus one evidence-based boundary per lane; on narrow screens, stack the three lane flows instead of forcing horizontal comparison.
- Keep `用户痛点` and `竞品分析` as two separate top-level chapters and navigation entries (`用户痛点` and `竞品分析`). Do not merge them into one wrapper chapter.
- Keep all navigation entries in the site header uniformly formatted as four characters (e.g. 产品概览、用户痛点、竞品分析、产品决策、AI系统、AI评测、用户测试、上线迭代、项目复盘、联系方式).
...
- Keep AI evaluation organized into three panels without a chapter subtitle: `评价体系` and `测评集设计` in the top row, and an expanded full-width `评测驱动优化` below detailing two engineering optimizations (range hallucination governance and high-order derived features). Avoid decorative miniature labels and unexplained A/B/C case IDs; do not append trailing summary metrics to the bottom of the optimization panel.
- Keep the evaluation set composition split into exactly two verified groups (128 contract cases in total, Coverage-v2): `全面训练分析集` (64 条用例) and `安全边界集` (64 条用例). Training analysis covers workout review, exercise trends, plateau diagnosis, frequency, plan adherence, and training-structure balance; move data insufficiency, evidence scope, medical safety, write permissions, prompt injection, and task-scope boundaries into the safety set.
...
- The system architecture (product development) chapter is removed per user decision. Do not re-introduce it.
